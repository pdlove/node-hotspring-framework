...
class HotspringServer {
    ...
    static async initialize(config) {
        ...
        // Activate all loaded stacks. Only the builtin stacks are loaded at this point.

        // #endregion


        // #region Load the remaining stacks from the disk and/or database. (Database loading not implemented yet.)
        ...
        try {
            ...
            if (await checkFolderExists(appStackPath))
                await HotspringStack.loadStacksFromPathAll(appStackPath);
        } catch (error) {
        ...
        await thisInstance.stacks['system'].activate(false); //Activate the system stack. This is required for the system stack to be loaded.
        ...
        for (const stackName in thisInstance.stacks) {
            ...
            if (stack.stackID == 0 && this.configuration.autoEnableStacks) 
                stack.enabled=true; //If this stack isn't in the database and autoEnable is true, enable it.
