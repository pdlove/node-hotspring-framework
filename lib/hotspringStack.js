...
class HotspringStack {
    ...
    try {
      ...
      for (const cls of classList) {
        if (cls.baseClass == 'HotspringModel') {
          ...
          newmodel.stack = newStack.name;
          newStack.models[newmodel.name] = newmodel;
          const routeList = newmodel.apiRoutes();
          for (const route of routeList) {
            newStack.routes[route.method + ':' + route.path] = route;
          }
          console.log("Loaded Object Type: " + newStack.name + "." + newmodel.name);
        }
  ...
  async activate() {
    ...
    for (const model in this.models) {
      ...
      global.hotspring.models[this.name + '.' + model] = this.models[model];
    }
