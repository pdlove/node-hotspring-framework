...
class HotspringStack {
  ...
  async activate() {
    ...
    const menuModel = global.hotspring.stacks['system'].models['menu'];
    for (const menuEntry of this.menuItems) {      
      ...
      const filter = { name: menuEntry.name, stackID: 1 }; //this.stackId };
      let menuResult = await menuModel.browseObjects({ filter });
      let menuDBItem = null;
      if (menuResult.length == 0) {
        menuDBItem = await menuModel.addObject({ stackID: 1, //this.stackId, 
