...
class HotspringStack {
  ...
  async activate(databaseWait = false) {
    ...
    if (!databaseWait) {
      await this.hotspring.sequelize.sync({ alter: true });
      await this.updateDatabaseObject(); //Update the stack object in the database so that the stackID is set as 
