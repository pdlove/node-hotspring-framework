...
class HotspringJob {
    ...
    static async updateDatabaseTemplateObject() {
        let model = await global.hotspring.stacks['system'].models['JobTemplate']
        let dbObject = await model.browseObjects({jobTemplateName: this.jobTemplateName, stack: this.stackName});
